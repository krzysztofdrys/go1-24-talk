...
info: |
  ## What is new in Go
  Presentation by Krzysztof Dryś
drawings:
  persist: false
css: unocss
wakeLock: "build"
...
The most interesting features in the last two releases.

Not always the most useful.

Starting with the most interesting ones, so I can finish at any time.

---

There was a time when Go was considered an almost complete language.

<Tweet id="700567036355678208" cards="hidden"/>
...
1. We get a performance boost for free.
2. It is interesting that hash maps is something that can be improved.
3. It's a bold step for Go developers to change the implementation of something as fundamental to the language as a map.
...
When I was a student, hash maps seemed like a solved problem to me: [^1].

<img src="./img/hash_map_wikipedia.png">
...
# Benchmark 1: append
Create a map and keep adding to it.
...
# Benchmark 2: insert 
Create a map with a predefined size and keep adding to it. Hopefully

```go
b.StopTimer()
m := make(map[int]struct{}, 2*size)
b.StartTimer()
for j := 0; j < size; j++ {
    m[j] = struct{}{}
}
...
Slidev is a slides maker and presenter designed for developers, consist of the  features
